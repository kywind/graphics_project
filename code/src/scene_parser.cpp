...
SceneParser::SceneParser(const char *filename) {
    ...
    ambient_color = Vector3f(0.5, 0.5, 0.5);
    num_lights = 0;
    ...
    if (num_lights + num_surface_lights == 0) {
...
void SceneParser::parseFile() {
    ...
    while (getToken(token)) {
        ...
        } else if (!strcmp(token, "ApertureCamera")) {
            parseApertureCamera();
        } else if (!strcmp(token, "Background")) {
            parseBackground();
        } else if (!strcmp(token, "Lights")) {
            parseLights();
        } else if (!strcmp(token, "SurfaceLights")) {
            parseSurfaceLights();
...
void SceneParser::parseApertureCamera() {
    char token[MAX_PARSER_TOKEN_LENGTH];
    // read in the camera parameters
    getToken(token);
    assert (!strcmp(token, "{"));
    ...
    int height = readInt();
    getToken(token);
    assert (!strcmp(token, "f_a"));
    float f_a = readFloat();
    getToken(token);
    assert (!strcmp(token, "u_a"));
    int u_a = readFloat();
    getToken(token);
    assert (!strcmp(token, "}"));
    camera = new ApertureCamera(center, direction, up, width, height, angle_radians, f_a, u_a);
}
...
void SceneParser::parseBackground() {
    ...
    while (true) {
        ...
        if (!strcmp(token, "}")) {
            break;
        } else if (!strcmp(token, "bg_color")) {
            background_color = readVector3f();
        } else if (!strcmp(token, "am_color")) {
...
void SceneParser::parseSurfaceLights() {
    char token[MAX_PARSER_TOKEN_LENGTH];
    getToken(token);
    assert (!strcmp(token, "{"));
    ...
    getToken(token);
    assert (!strcmp(token, "numLights"));
    num_surface_lights = readInt();

    surface_lights = new SurfaceLight *[num_surface_lights];

    // read in the objects
    int count = 0;
    while (num_surface_lights > count) {
        getToken(token);
        if (strcmp(token, "SurfaceLight") == 0) {
            surface_lights[count] = parseSurfaceLight();
        }
        else {
            printf("Unknown token in parseSurfaceLight: '%s'\n", token);
            exit(0);
        }
        count++;
    }
...
SurfaceLight *SceneParser::parseSurfaceLight() {
    char token[MAX_PARSER_TOKEN_LENGTH];
    getToken(token);
    assert (!strcmp(token, "{"));
    ...
    getToken(token);
    assert (!strcmp(token, "norm"));
    Vector3f norm = readVector3f();
    ...
    getToken(token);
    assert (!strcmp(token, "x_len"));
    float x_len = readFloat();

    getToken(token);
    assert (!strcmp(token, "y_len"));
    float y_len = readFloat();

    getToken(token);
    assert (!strcmp(token, "}"));

    return new SurfaceLight(position, color, norm, x_axis, x_len, y_len);
}
...
Texture *SceneParser::parseTexture() {
    ...
    assert (!strcmp(token, "Material"));
    Material* m = parseMaterial();
    Texture* answer = new Texture(m);
    getToken(token);

    while (true) {
        if (!strcmp(token, "diffusion")) {
            answer->use_diffuse_map = true;
            getToken(token);
            answer->read_diffuse_map(token);
        } 
        else if (!strcmp(token, "specular")) {
            answer->use_specular_map = true;
            getToken(token);
            answer->read_specular_map(token);
        }
        else if (!strcmp(token, "bump")) {
            answer->use_bump_map = true;
            getToken(token);
            answer->read_bump_map(token);
        }
        else if (!strcmp(token, "ambient")) {
            answer->use_ambient_map = true;
            getToken(token);
            answer->read_ambient_map(token);
        }
        else if (!strcmp(token, "emission")) {
            answer->use_emission_map = true;
            getToken(token);
            answer->read_emission_map(token);
        }
        else break;
        getToken(token);
    }
...
Material *SceneParser::parseMaterial() {
    ...
    float w_diffusion = 1;
    float w_reflection = 0;
    float w_transmission = 0;
    float ref_index = 1;
    bool is_light = false;
    ...
    while (true) {
        ...
        } else if (strcmp(token, "w_diffusion") == 0) {
            // Optional: read in texture and draw it.
            w_diffusion = readFloat();
        } else if (strcmp(token, "w_reflection") == 0) {
            // Optional: read in texture and draw it.
            w_reflection = readFloat();
        } else if (strcmp(token, "w_transmission") == 0) {
            // Optional: read in texture and draw it.
            w_transmission = readFloat();
        } else if (strcmp(token, "ref_index") == 0) {
            // Optional: read in texture and draw it.
            ref_index = readFloat();
        } else {
            assert (!strcmp(token, "}"));
    ...
    auto *answer = new Material(diffuseColor, specularColor, shininess, ambientColor, \
                                w_diffusion, w_reflection, w_transmission, ref_index);
    return answer;
}
...
Mesh *SceneParser::parseTriangleMesh() {
    ...
    while (true) {
        if (!strcmp(token, "obj_file")) {
            ...
            use_texture = true;
            int index = readInt();
            assert (index >= 0 && index <= getNumTextures());
    ...
    Mesh *answer = new Mesh(filename, interpolate, current_material);
    if (use_texture) answer->set_texture(texture);
    return answer;
}
